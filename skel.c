...
#define MASK_16   0XFFFF       /* Mask value for bit 16 */
...
void build_ether_header(struct ether_header *eth_hdr, uint8_t *new_mac_src, uint8_t *new_mac_dest, uint16_t type)
{
	...
	memcpy(eth_hdr->ether_dhost, new_mac_dest, ETH_ALEN);
	memcpy(eth_hdr->ether_shost, new_mac_src, ETH_ALEN);
}

void print_mac(struct ether_header *eth_hdr)
{
	printf("Packet is of type: %d\n", eth_hdr->ether_type);
