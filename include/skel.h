...
/**
 * @brief Parses an arp header starting from a buffer if the type
 * of the ether header is ARP.
 * @param buffer 
...
/**
 * @brief Parses and icmp header starting from a buffer if
 * the type of ether is IP and the actual packet is an ICMP.
 * @param buffer 
...
/**
 * @brief Copy the type, the mac source and the mac destination given
 * into a ethernet header given as parameter.
 * @param eth_hdr 
 * @param new_mac_src 
 * @param new_mac_dest 
 * @param type 
 */
void build_ether_header(struct ether_header *eth_hdr, uint8_t *new_mac_src, uint8_t *new_mac_dest, uint16_t type);

/**
 * @brief Used for debugg, nice function to have that shows the
 * ethernet header fields from the corresponding packet.
 * @param eth_hdr 
 */
void print_mac(struct ether_header *eth_hdr);
